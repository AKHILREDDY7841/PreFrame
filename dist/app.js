...
const localRepo = new LocalProjectRepository();
const cloudRepo = new CloudProjectRepository();
let repo = cloudRepo;
let preview = false;
let errorMessage = "";
let accountBadge = "Free";
const root = document.querySelector("#app");
const base = location.pathname.startsWith("/PreFrame") ? "/PreFrame" : "";
...
const link = (p, l, c = "") => `<a class="${c}" href="${href(p)}" data-route>${l}</a>`;
...
async function home() {
    ...
    const tile = (title, subtitle, icon, route) => `<article class="tool-tile"><span class="tool-icon" aria-hidden="true">${icon}</span><h3>${title}</h3><p>${subtitle}</p>${first ? link(`/app/projects/${first.id}/${route}`, "Open →", "text-link") : "<span class='quiet'>Create a project first</span>"}</article>`;
...
function dashboard(p) {
    const group = (name) => `<section class="dashboard-group"><h2>${name}</h2><div class="module-grid">${tools.filter(([c]) => c === name).map(([, label, t]) => `<article><h3>${label}</h3><p>Empty workspace: this module has not been implemented yet.</p>${link(`/app/projects/${p.id}/${t}`, "Open workspace", "text-link")}</article>`).join("")}</div></section>`;
...
async function render() {
    ...
    if (query.get("preview") === "1")
        sessionStorage.setItem("preframe-preview", "1");
    ...
    if (session && !preview && r.page !== "landing" && r.page !== "auth") {
        try {
            const identity = await cloudRepo.getIdentity();
            accountBadge = identity?.isAdmin ? "Admin" : identity?.profile.tier === "premium" ? "Premium" : "Free";
        }
        catch {
            accountBadge = "Free";
        }
    ...
    try {
        if (r.page === "landing")
            ...
            history.replaceState({}, "", href("/auth"));
            root.innerHTML = auth();
        }
        else if (r.page === "home") {
            ...
            if (p && r.page === "project")
                wire(p);
            if (p && r.page === "workspace" && r.tool === "import")
                wireImport();
        }
    }
    catch (e) {
        root.innerHTML = shell(`<main class="empty-workspace"><h1>Could not load Preframe</h1><p>${esc(e instanceof Error ? e.message : "Unknown error")}</p>${link("/app", "Retry", "button")}</main>`, !!session || preview);
    }
    document.querySelectorAll("[data-route]").forEach(a => a.addEventListener("click", e => { if (a.origin === location.origin) {
        e.preventDefault();
    ...
    document.querySelector("#eye")?.addEventListener("click", () => { const warm = document.body.classList.toggle("eye-saver"); localStorage.setItem("preframe-eye-saver", String(warm)); document.querySelector("#eye").setAttribute("aria-pressed", String(warm)); });
    if (localStorage.getItem("preframe-eye-saver") === "true")
    ...
    form?.addEventListener("submit", async (event) => { event.preventDefault(); try {
        ...
        const id = await cloudRepo.createProject(title, Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC");
        history.pushState({}, "", href(`/app/projects/${id}`));
        errorMessage = "";
        render();
    }
    catch (e) {
        errorMessage = e instanceof Error ? e.message : "Could not create project";
        render();
    } });
    if (document.querySelector("#invitations-list"))
        loadInvitations();
}
async function loadInvitations() { const target = document.querySelector("#invitations-list"); if (!target)
    return; try {
    const rows = await cloudRepo.invitations();
    target.innerHTML = rows.length ? rows.map(row => `<p>Project invitation <button type="button" data-accept="${esc(row.id)}">Accept</button></p>`).join("") : "<p>No pending invitations.</p>";
    target.querySelectorAll("[data-accept]").forEach(button => button.addEventListener("click", async () => { try {
        const id = await cloudRepo.acceptInvitation(button.dataset.accept);
        history.pushState({}, "", href(`/app/projects/${id}`));
        render();
    }
    catch (e) {
        errorMessage = e instanceof Error ? e.message : "Could not accept invitation";
        render();
    } }));
}
catch (e) {
    target.textContent = e instanceof Error ? e.message : "Could not load invitations";
} }
async function renderTierPanel() {
    ...
    if (accountBadge === "Free")
        panel.innerHTML = "<strong>Free plan</strong><p>One active owned project. Finish or archive it before creating another.</p>";
...
function wireImport() {
    const input = document.querySelector("#backup-file"), target = document.querySelector("#backup-preview");
    input?.addEventListener("change", async () => {
        if (!target || !input.files?.[0])
            return;
        target.textContent = "Validating archive…";
        try {
            if (input.files[0].size > 50_000_000)
                throw new Error("Archive is too large to preview safely");
            ...
            target.textContent = e instanceof Error ? e.message : "Archive validation failed";
        }
    });
}
async function wire(p) {
    const input = document.querySelector("#project-title"), status = document.querySelector("#save-status");
    let current = p;
    let timer;
    let sync;
    if (!preview) {
        ...
        if (user) {
            sync = new DurableProjectSync(user.id, p.id, cloudRepo, (state, detail, remote) => { status.textContent = { "saved-locally": "Saved locally", syncing: "Syncing", synced: "Synced", "sync-failed": "Sync failed", conflict: "Conflict" }[state] + (detail ? ` — ${detail}` : ""); if (remote && state === "synced")
                current = remote; });
            const pending = await sync.restore();
            if (pending) {
                input.value = pending.title;
                status.textContent = "Saved locally";
            }
            addEventListener("online", () => sync?.flush());
        }
    }
    input.addEventListener("input", () => { const title = input.value.trim(); if (!title) {
        status.textContent = "Title cannot be empty";
        return;
    } if (preview) {
        status.textContent = "Saving locally";
        clearTimeout(timer);
        timer = setTimeout(async () => { const result = await localRepo.saveProject({ ...current, title }, current.revision); if (result.kind === "ok") {
            current = result.value.value;
            status.textContent = "Saved locally";
        }
        else
            status.textContent = "Conflict"; }, 650);
    }
    else
        sync?.stage({ ...current, title }, current.revision).catch(e => status.textContent = `Local save failed — ${e.message}`); });
    const invite = document.querySelector("#invite-editor");
    invite?.addEventListener("submit", async (event) => { event.preventDefault(); try {
        await cloudRepo.inviteEditor(p.id, String(new FormData(invite).get("email")));
        errorMessage = "Invitation created. The editor can accept it after signing in with that email.";
        render();
    }
    catch (e) {
        errorMessage = e instanceof Error ? e.message : "Could not invite editor";
        render();
    } });
    const members = document.querySelector("#members");
    if (members)
        cloudRepo.members(p.id).then(rows => { members.innerHTML = rows.map(row => `<p>${esc(row.user_id)} · ${esc(row.role)} ${row.role === "editor" ? `<button type="button" data-remove="${esc(row.user_id)}">Remove</button>` : ""}</p>`).join(""); members.querySelectorAll("[data-remove]").forEach(button => button.addEventListener("click", async () => { try {
            await cloudRepo.removeEditor(p.id, button.dataset.remove);
            errorMessage = "Editor removed.";
            render();
        }
        catch (e) {
            errorMessage = e instanceof Error ? e.message : "Could not remove editor";
            render();
    ...
    document.querySelector("#export-project")?.addEventListener("click", async () => {
        ...
        try {
            const archive = await exportProject(p.id);
            const blob = new Blob([JSON.stringify(archive)], { type: "application/json" });
            const url = URL.createObjectURL(blob);
            const anchor = document.createElement("a");
            anchor.href = url;
            anchor.download = `preframe-${p.id}-${new Date().toISOString().slice(0, 10)}.json`;
            anchor.click();
            setTimeout(() => URL.revokeObjectURL(url), 30_000);
            status.textContent = "Archive downloaded. Unsynced local edits and unuploaded originals are excluded.";
        }
        catch (e) {
            status.textContent = e instanceof Error ? e.message : "Export failed";
        }
    });
}
addEventListener("popstate", render);
supabase.auth.onAuthStateChange((event) => { if (event === "SIGNED_IN" && sessionStorage.getItem("preframe-oauth-pending") === "1") {
    sessionStorage.removeItem("preframe-oauth-pending");
    history.replaceState({}, "", href("/app"));
    render();
}
else if (location.pathname.endsWith("/auth") || location.pathname.endsWith("/app"))
    render(); });
render();
