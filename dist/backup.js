...
export async function previewBackup(raw) {
    ...
    if (!backup || backup.manifest?.format !== "preframe-project" || backup.manifest.version !== 1
        ...
        || backup.content.projects.length !== 1)
        throw new Error("Unsupported or incomplete Preframe archive");
    for (const table of projectTables)
        if (!Array.isArray(backup.content[table]))
            throw new Error(`Missing ${table} records`);
    if (!Array.isArray(backup.content.character_actors))
        throw new Error("Missing cast links");
    const project = backup.content.projects[0];
    if (project.id !== backup.manifest.projectId || typeof project.title !== "string")
        throw new Error("Project manifest does not match content");
    for (const table of projectTables) {
        for (const row of backup.content[table])
            if (!row || typeof row !== "object" || row.project_id !== project.id)
                throw new Error(`Cross-project or invalid ${table} record`);
    }
    const characterIds = new Set(backup.content.characters.map(row => row.id));
    const actorIds = new Set(backup.content.actors.map(row => row.id));
    for (const link of backup.content.character_actors)
    ...
    for (const file of backup.files) {
        ...
        if (bytes.length !== file.bytes || await sha256(bytes) !== file.sha256)
            throw new Error(`Media checksum does not match: ${file.path}`);
    }
    const mediaPaths = new Set(backup.content.media.map(row => row.storage_path));
    if (mediaPaths.size !== paths.size || [...mediaPaths].some(path => !paths.has(path)))
        throw new Error("Media manifest is incomplete");
