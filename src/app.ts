...
async function home(view: "home" | "projects" | "shared" | "settings" | "recycle" = "home"){
  ...
  const activity=await Promise.all(projects.flatMap(project=>tools.map(async([,label,tool])=>{
    const records=await toolRecords(preview?"local-demo-owner":currentUserId,project.id,tool as ToolName);
    return records.map(record=>({title:record.title||label,detail:`${label} · ${project.title}`,href:href(`/app/projects/${project.id}/${tool}`),occurredAt:record.updatedAt}));
  })));
...
function dashboard(p:Project){
  ...
  return shell(`<main class="app-shell project-hub-shell"><aside class="side-nav">${link("/app","← Back to home")}<span>PROJECT WORKSPACE</span></aside><section class="project-page project-hub"><div class="project-hub-heading"${coverStyle}><p class="eyebrow">PREFRAME / PROJECT</p><div class="project-heading-row"><h1>${esc(p.title)}</h1><button id="project-settings-toggle" type="button">Edit project</button></div><p>From first draft to shooting day.</p></div><div class="project-bands">${group("Write")}${group("Visualize")}${group("Plan")}</div><details id="project-management" class="project-management"><summary>Project settings, backups and collaborators</summary><div class="project-management-body"><label class="project-title">Project title <input id="project-title" value="${esc(p.title)}" maxlength="160"></label><section class="project-cover-settings"><h2>Project cover</h2><p>Choose a landscape image. PreFrame optimizes it before saving.</p><label class="cover-upload">Choose cover image <input id="project-cover-file" type="file" accept="image/jpeg,image/png,image/webp"></label><p id="cover-status" role="status"></p></section><div id="save-status" role="status">${preview?"Saved locally":"Synced"}</div><p role="alert">${esc(errorMessage)}</p>${!preview&&p.ownerId===currentUserId?`<section class="project-danger-zone"><h2>Delete project</h2><p>Moves this project to Recycle bin for 15 days. You can restore it until then.</p><button id="delete-project" type="button">Move to Recycle bin</button></section>`:""}${preview?"":`<section class="backup-panel"><h2>Project backup</h2><p>Export synced project data and uploaded media. Unsynced local edits and original files not uploaded are excluded.</p><button id="export-project" type="button">Export project archive</button><p id="export-status" role="status"></p></section>`}<section class="backup-panel local-backup-panel"><h2>Cloud tool data</h2><p>Screenplay, notes, shots, storyboards, schedules, locations and call-sheet drafts sync to this project. A local copy is also retained for offline recovery.</p><div class="local-backup-actions"><button id="export-local-tools" type="button">Download project tool backup</button><label>Restore tool backup <input id="import-local-tools" type="file" accept=".json,application/json"></label></div><p id="local-backup-status" role="status"></p></section>${preview?"":`<section class="dashboard-group"><h2>Collaborators</h2><form id="invite-editor"><label>Invite editor by email <input name="email" type="email" required></label><button type="submit">Invite</button></form><div id="members">Loading…</div></section>`}</div></details></section></main>`,true);
}
function workspace(p:Project,t:string){if(t!=="import"&&t!=="members")return shell(toolWorkspace(p,t,href),true,accountBadge,true);const label=t==="import"?"Import Script":"Project collaborators";return shell(`<main class="workspace-page"><div class="workspace-head">${link(`/app/projects/${p.id}`,"← Back to project","plain-link")}<span class="project-chip">${esc(p.title)}</span></div><section class="empty-workspace"><p class="eyebrow">PROJECT WORKSPACE</p><h1>${label}</h1>${t==="import"?`<p>Preview a Preframe archive without changing this project.</p><label class="import-label">Choose a Preframe archive (.json) <input id="backup-file" type="file" accept=".json,application/json"></label><div id="backup-preview" role="status"></div>`:"<p>Manage collaborators from the project overview.</p>"}</section></main>`,true,accountBadge,true);}
...
async function render(){
  ...
  }else if(preview)accountBadge=query.get("home")==="premium"?"Premium":"Preview";
  try{
    ...
    else if(r.page==="home"||r.page==="projects"||r.page==="shared"||r.page==="settings"||r.page==="recycle")root.innerHTML=await home(r.page);
    else if(r.page==="collaborate"){
      const identity=await repo.getIdentity();
      const premium=preview ? new URLSearchParams(location.search).get("home")==="premium" : Boolean(identity?.isAdmin||identity?.profile.tier==="premium");
      if(!premium)root.innerHTML=premiumBlocked("Collaboration");
      else {root.innerHTML=collaborationPage((await repo.listProjects()).filter(p=>p.ownerId===identity?.profile.id),href,preview);wireCollaboration(href,url=>{history.pushState({},"",url);render();});}
    }
...
async function loadRecycleBin(){
  ...
  try{
    const projects=await cloudRepo.recycleBin();
...
async function wire(p:Project){
  ...
  document.querySelector("#delete-project")?.addEventListener("click",async()=>{
    ...
    try{await cloudRepo.deleteProject(p.id);errorMessage="Project moved to Recycle bin. It can be restored for 15 days.";history.pushState({},"",href("/app/recycle"));render();}
