...
function importProjectPicker(projects:Project[]){return shell(`<main class="workspace-page"><div class="workspace-head">${link("/app","← Back to home","plain-link")}<span class="project-chip">SCRIPT IMPORT</span></div><section class="empty-workspace import-project-picker"><p class="eyebrow">PROJECT WORKSPACE</p><h1>Where should this script go?</h1><p>Choose the project whose screenplay should receive the PDF or PreFrame archive.</p>${projects.length?`<form id="import-project-form"><label for="import-project-select">Project<select id="import-project-select" name="projectId">${projects.map(project=>`<option value="${esc(project.id)}">${esc(project.title)}</option>`).join("")}</select></label><button type="submit">Continue to import</button></form>`:`<p>Create a project before importing a script.</p>${link("/app","Back to dashboard","button")}`}</section></main>`,true,accountBadge,true);}
...
async function render(){
  ...
  try{
    ...
    else if(r.page==="import")root.innerHTML=importProjectPicker(await repo.listProjects());
  ...
  document.querySelector<HTMLFormElement>("#import-project-form")?.addEventListener("submit",event=>{event.preventDefault();const form=event.currentTarget as HTMLFormElement;const projectId=String(new FormData(form).get("projectId")||"");if(!projectId)return;history.pushState({},"",href(`/app/projects/${projectId}/import`));render();});
