...
export function renderHomePage({ logo, href, projects, name, badge, premium, preview, error, upcoming, recentActivity, view = "home" }: HomeOptions): string {
  ...
  const nav = `<aside class="home-sidebar" aria-label="App navigation"><nav>${route(href("/app"), `${icon("home")}<span>Home</span>`, "active")}<a href="${href("/app/projects")}" data-route>${icon("projects")}<span>Projects</span></a><a href="${href("/app/shared")}" data-route>${icon("shared")}<span>Shared &amp; collaboration</span></a><a href="${href("/app/recycle")}" data-route>${icon("projects")}<span>Recycle bin</span></a></nav><p>Plan better.<br>Shoot greater.</p><a class="home-settings-link" href="${href("/app/settings")}" data-route>${icon("settings")}<span>Settings</span></a></aside>`;
  ...
  if (view !== "home") return `${header}<main class="home-shell collection-shell">${navigation}<div class="home-main collection-main"><div class="collection-heading"><p class="eyebrow">YOUR WORKSPACE</p><h1>${view === "projects" ? "Projects" : view === "shared" ? "Shared & collaboration" : view === "recycle" ? "Recycle bin" : "Settings"}</h1><p>${view === "projects" ? "Your stories, ready for the next step." : view === "shared" ? "Projects you create together. Collaboration is a Premium feature." : view === "recycle" ? "Projects stay here for 15 days before permanent deletion." : "Your account and workspace."}</p></div><section class="collection-content"><p role="alert">${escapeHtml(error)}</p>${view === "settings" ? `<h2>Account</h2><p>${escapeHtml(name)} · ${escapeHtml(badge)}</p><p>Eye saver is available inside Screenplay and Docs & Notes.</p>` : view === "recycle" ? `<div id="recycle-list">Loading recycle bin…</div>` : `<div class="home-section-heading"><h2>${view === "projects" ? "Owned projects" : "Shared projects"}</h2>${view === "projects" ? projectForm : premium ? route(href('/app/collaborate'), 'Collaborate with a code', 'button') : '<button type="button" data-premium-feature="Collaboration">Collaborate with a code</button>'}</div><div class="home-project-grid">${cards}</div><p id="project-search-empty" hidden>No matching projects.</p>${view === "shared" && !preview ? '<section class="home-lower"><h2>Pending invitations</h2><div id="invitations-list">Loading…</div></section>' : ''}`}</section></div></main>`;
