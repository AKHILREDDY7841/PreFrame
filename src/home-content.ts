...
type HomeOptions = {
  view?: "home" | "projects" | "shared" | "settings";
  logo: string;
...
export function renderHomePage({ logo, href, projects, name, badge, premium, preview, error, upcoming, recentActivity, view = "home" }: HomeOptions): string {
  ...
  const nav = `<aside class="home-sidebar" aria-label="App navigation"><nav>${route(href("/app"), `${icon("home")}<span>Home</span>`, "active")}<a href="${href("/app/projects")}" data-route>${icon("projects")}<span>Projects</span></a><a href="${href("/app/shared")}" data-route>${icon("shared")}<span>Shared with me</span></a>${route(href("/app/collaborate"), `${icon("shared")}<span>Collaborate</span>`)}<span class="home-nav-divider"></span><a href="${href("/app/settings")}" data-route>${icon("settings")}<span>Settings</span></a></nav><p>Plan better.<br>Shoot greater.</p></aside>`;
  ...
  const planNotice = !premium ? `<div class="home-plan-notice">${icon("info")}<p><strong>You're on the Free plan.</strong> One active owned project is included.${projects.length ? " Premium checkout is coming soon." : " Create a project to get started."}</p></div><section class="home-donation" aria-label="Support PreFrame"><div><p class="eyebrow">SUPPORT PREFRAME</p><h3>Help keep the Free plan available.</h3><p>Donations will be available here once the secure payment checkout is connected.</p></div><span aria-label="Donations opening with secure checkout">Coming soon</span></section>` : firstProject ? `<div class="home-collaborate">${icon("shared")}<div><strong>Collaborate</strong><p>Work with your crew. Invite editors and manage access in your project.</p></div>${route(href(`/app/projects/${firstProject.id}`), "Open project →", "home-collaborate-link")}</div>` : "";
  const quickActions = firstProject ? `<div class="home-quick-actions"><a href="${href("/app/collaborate")}" data-route>${icon("shared")}<span><strong>Collaborate</strong><small>Invite a teammate or join with a code.</small></span><b>→</b></a><a href="${href(`/app/projects/${firstProject.id}/import`)}" data-route>${icon("import")}<span><strong>Import script</strong><small>Bring an existing PreFrame backup into view.</small></span><b>→</b></a></div>` : "";
  ...
  if (view !== "home") return `${header}<main class="home-shell collection-shell">${navigation}<div class="home-main collection-main"><div class="collection-heading"><p class="eyebrow">YOUR WORKSPACE</p><h1>${view === "projects" ? "Projects" : view === "shared" ? "Shared with me" : "Settings"}</h1><p>${view === "projects" ? "Your stories, ready for the next step." : view === "shared" ? "The projects you create together." : "Your account and workspace."}</p></div><section class="collection-content"><p role="alert">${escapeHtml(error)}</p>${view === "settings" ? `<h2>Account</h2><p>${escapeHtml(name)} · ${escapeHtml(badge)}</p><p>Eye saver is available inside Screenplay and Docs & Notes.</p>` : `<div class="home-section-heading"><h2>${view === "projects" ? "Owned projects" : "Shared projects"}</h2>${view === "projects" ? projectForm : route(href('/app/collaborate'), 'Join with a code', 'button')}</div><div class="home-project-grid">${cards}</div><p id="project-search-empty" hidden>No matching projects.</p>${view === "shared" && !preview ? '<section class="home-lower"><h2>Pending invitations</h2><div id="invitations-list">Loading…</div></section>' : ''}`}</section></div></main>`;
