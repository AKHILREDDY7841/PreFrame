...
function projectCard(project: Project, href: HomeOptions["href"], index: number) {
  ...
  return `<a class="home-project-card" href="${href(`/app/projects/${project.id}`)}" data-route data-project-title="${escapeHtml(project.title.toLocaleLowerCase())}"><span class="home-project-image home-project-image-${index % 4}" aria-hidden="true"></span><span class="home-project-copy"><strong>${escapeHtml(project.title)}</strong><small>${updated}</small></span><span class="home-project-arrow" aria-hidden="true">↗</span></a>`;
}

export function renderHomePage({ logo, href, projects, name, badge, premium, preview, error }: HomeOptions): string {
  const firstName = name.trim().split(/\s+/)[0] || "there";
  const initial = firstName.slice(0, 1).toLocaleUpperCase();
  const firstProject = projects[0];
  const tool = (label: string, detail: string, iconName: string, path: string) => {
    const body = `${icon(iconName)}<strong>${label}</strong><small>${detail}</small><span class="home-tool-arrow" aria-hidden="true">→</span>`;
    return firstProject ? route(href(`/app/projects/${firstProject.id}/${path}`), body, "home-tool") : `<div class="home-tool home-tool-disabled" aria-disabled="true">${body}</div>`;
  };
  ...
  const eye = `<button id="eye" class="home-eye" type="button" aria-pressed="false">${icon("eye")}<span>Eye saver</span><span class="home-eye-switch" aria-hidden="true"></span></button>`;
  ...
  const nav = `<aside class="home-sidebar" aria-label="App navigation"><nav>${route(href("/app"), `${icon("home")}<span>Home</span>`, "active")}<a href="#projects">${icon("projects")}<span>Projects</span></a><a href="#invitations">${icon("shared")}<span>Shared with me</span></a><span class="home-nav-divider"></span><a href="#account-settings">${icon("settings")}<span>Settings</span></a></nav><p>Plan better.<br>Shoot greater.</p></aside>`;
  ...
  const schedule = `<section class="home-schedule" aria-labelledby="schedule-title"><div class="home-schedule-list"><div class="home-schedule-heading"><h2 id="schedule-title">Upcoming Schedule</h2>${firstProject ? route(href(`/app/projects/${firstProject.id}/schedule`), "Open schedule →") : ""}</div><div class="home-schedule-empty"><span class="home-schedule-date" aria-hidden="true">—</span><div><strong>No shoot days scheduled yet.</strong><p>Open a project when you're ready to plan your production.</p></div></div></div><div class="home-schedule-image" role="img" aria-label="${premium ? "Director's chair and cinema camera overlooking mountains" : "Quiet alpine lake at sunrise"}"></div></section>`;
  return `<header class="home-topbar">${logo}<div class="home-topbar-controls">${search}${eye}${account}</div></header><main class="home-shell ${premium ? "home-premium" : "home-free"}">${nav}<div class="home-main"><section class="home-banner" aria-label="Welcome"><div class="home-banner-copy"><p>Good morning,</p><h1>${escapeHtml(firstName)}.</h1><p>Ideas look better in focus.</p></div><p class="home-banner-quote">Good ideas<br>deserve a plan.</p></section><div class="home-main-inner"><section class="home-projects" id="projects" aria-labelledby="home-projects-title"><div class="home-section-heading"><h2 id="home-projects-title">Your ${premium ? "Projects" : "Project"}</h2><div class="home-project-actions">${premium && projects.length > 4 ? `<a href="#projects">View all →</a>` : ""}${projectForm}</div></div><p class="home-error" role="alert">${escapeHtml(error)}</p><div class="home-project-grid">${cards}</div><p id="project-search-empty" class="home-search-empty" hidden>No projects match your search.</p>${planNotice}</section><section class="home-tools" aria-labelledby="home-tools-title"><h2 id="home-tools-title">Tools</h2><div class="home-tool-grid">${tool("Write", "Screenplays & Notes", "write", "screenplay")}${tool("Visualize", "Shot lists & Storyboards", "visualize", "shots")}${tool("Plan", "Schedules & Call sheets", "plan", "schedule")}${tool("Import Script", "Bring in your script", "import", "import")}</div></section>${schedule}<section class="home-lower" id="invitations"><h2>Shared with me</h2>${preview ? "<p>Sign in to see invitations to shared projects.</p>" : '<div id="invitations-list">Loading invitations…</div>'}</section><section class="home-lower home-account-settings" id="account-settings"><h2>Account settings</h2><p>${escapeHtml(name)} · ${escapeHtml(badge)}${preview ? " · Local preview" : ""}</p></section></div></div></main>`;
}
