...
export function studioShell(id: string, title: string, name: string, label: string, content: string, href: (path: string) => string): string {
  ...
  return `<main class="tool-page studio-workspace" data-tool="${esc(name)}" data-project="${esc(id)}">
    <aside class="studio-sidebar"><a class="studio-brand" href="${href('/app')}" data-route>PREFRAME</a><nav aria-label="Workspace navigation">${nav('/app/projects', 'Projects', 'projects')}${nav(root, 'Project overview', 'projects')}<span class="studio-nav-caption">PROJECT TOOLS</span>${(['screenplay', 'notes', 'shots', 'storyboards', 'schedule', 'calendar', 'call-sheets', 'locations'] as const).map(tool => nav(`${root}/${tool}`, ({screenplay:'Screenplay',notes:'Docs & Notes',shots:'Shot Lists',storyboards:'Storyboards',schedule:'Production Schedule',calendar:'Calendar','call-sheets':'Call Sheets',locations:'Locations'})[tool], tool, tool === name)).join('')}<span class="studio-nav-divider"></span>${nav(root + '#project-management', 'People & settings', 'people')}</nav></aside>
    <div class="studio-main"><header class="studio-banner"><div class="studio-breadcrumb"><a href="${href('/app')}" data-route>Projects</a><span>/</span><a href="${href(root)}" data-route>${esc(title)}</a></div><h1>${esc(title)}</h1><div class="studio-current-tool">${studioIcon(name)}<span>${label}</span></div><a class="studio-project-button" href="${href(root)}" data-route>Project overview ↗</a></header>${content}</div></main>`;
}
