...
export function studioShell(id: string, title: string, name: string, label: string, content: string, href: (path: string) => string): string {
  ...
  return `<main class="tool-page studio-workspace" data-tool="${esc(name)}" data-project="${esc(id)}">
    ...
    <div class="studio-main"><div class="studio-utility-row"><button type="button" class="studio-sidebar-toggle" id="studio-sidebar-toggle" aria-expanded="false" aria-controls="studio-sidebar-panel">☰ <span>Tools</span></button><a class="studio-project-button" href="${href(root)}" data-route>Project overview ↗</a></div>${content}</div></main>`;
}
