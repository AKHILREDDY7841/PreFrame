...
export function studioShell(id: string, title: string, name: string, label: string, content: string, href: (path: string) => string): string {
  ...
  return `<main class="tool-page studio-workspace" data-tool="${esc(name)}" data-project="${esc(id)}">
    ...
    <div class="studio-main"><header class="studio-banner"><div class="studio-breadcrumb"><a href="${href('/app')}" data-route>Projects</a><span>/</span><a href="${href(root)}" data-route>${esc(title)}</a></div><h1>${esc(title)}</h1><div class="studio-current-tool">${studioIcon(name)}<span>${label}</span></div><a class="studio-project-button" href="${href(root)}" data-route>Project overview ↗</a></header><div class="studio-utility-row"><button type="button" class="studio-sidebar-toggle" id="studio-sidebar-toggle" aria-expanded="false" aria-controls="studio-sidebar-panel">☰ <span>Tools</span></button></div>${content}</div></main>`;
}
